...
module.exports = {
    ...
    //          - cashier (id_pegawai, id_order, id_cara_bayar, total_bayar)
    //          - customer (id_order, id_cara_bayar, total_bayar)
    ...
    createNewPayment: (req, res) => {
        ...
        Order.selectTotalPayment(req.body.id_order, result => { 
            value.push(result[0].total)
            // Order from Admin/Cashier or Customer
            if (tipe == 1 || tipe == 2) {
                Payment.addPaymentFromCashier(value, data => {
                    res.status(200).send("Successfully add new payment.")
                })
            } else if (tipe == 5) {
    ...
    updatePaymentDone: (req, res) => {
        const value = Object.values(req.body);
        Payment.updatePaymentDone(value, req.params.id, result => {
